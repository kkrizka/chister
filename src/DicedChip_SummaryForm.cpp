...
void DicedChip_SummaryForm::setResults(const QList<DicedChipSlot*>& results)
{
  for(const DicedChipSlot *slot : results)
    {
      QString colcode=(slot?"#00FF00":"#FF0000");
      m_slotLabels[slot->slot()]->setStyleSheet("QLabel { background-color:"+colcode+"; }");
    }
}
